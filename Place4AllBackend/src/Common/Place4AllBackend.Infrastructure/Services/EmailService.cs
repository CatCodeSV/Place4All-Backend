...
namespace Place4AllBackend.Infrastructure.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task SendAsync(EmailRequest request)
        {
            ...
            {
                _logger.LogError(ex, "Place4AllBackend EmailService: Unhandled Exception for Request {@Request}",
                    request);
            }

            _logger.LogWarning(
                $"Sending email to {request.ToMail} from {request.FromMail} with subject {request.Subject}.");
        }
    }
}