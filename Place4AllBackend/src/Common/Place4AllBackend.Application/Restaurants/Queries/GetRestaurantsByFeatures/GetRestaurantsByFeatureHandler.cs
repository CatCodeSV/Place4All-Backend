    ...
    GetRestaurantsByFeatureHandler : IRequestHandlerWrapper<GetRestaurantsByFeatureQuery, List<RestaurantSummarizedDto>>
{
    ...
    {
        var list = await _context.Restaurants.Where(r => r.Features.Any(f => request.Features.Contains(f.Id))).Select(
            r => new RestaurantSummarizedDto()
            {
                Id = r.Id,
                ...
                Rating = r.Reviews.Average(rv => rv.Value),
